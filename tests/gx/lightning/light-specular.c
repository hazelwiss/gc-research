...
static f32 light_rot_z = 0.0;
static f32 light_rot_y = -3.14 / 2;
static GXColor ambient = {0x10, 0x10, 0x10, 0xff};
...
static void configure_light(Mtx view) {
  ...
  Mtx rot_z, rot_y;
  guMtxRotRad(rot_z, 'Z', light_rot_z);
  ...
  GXLightObj lobj;
  GX_InitSpecularDir(&lobj, ldir.x, ldir.y, ldir.z);
  GX_InitLightShininess(&lobj, 100.0);
  ...
  Mtx model, modelview;
  guMtxIdentity(model);
  guMtxScaleApply(model, model, 0.035, 0.035, 0.035);
  guMtxConcat(rot_z, model, model);
  guMtxConcat(rot_y, model, model);
  guMtxTransApply(model, model, -25, 0, 0);
  guMtxConcat(view, model, modelview);
  GX_LoadPosMtxImm(modelview, GX_PNMTX0);

  GX_Begin(GX_TRIANGLES, GX_VTXFMT1, arrow_model_len);
  for (int i = 0; i < arrow_model_len; ++i) {
    struct ModelVertex *vtx = &arrow_model[i];
    GX_Position3f32(vtx->pos.x, vtx->pos.y, vtx->pos.z);
    GX_Color3f32(0.7, 0.7, 0.0);
  }
  GX_End();
